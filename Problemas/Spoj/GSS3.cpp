...
void updateNode(node *tree,int index,int s,int e,int pos,int val)
{
    if(pos<s || pos>e) return ;
    if(s==e)
    {
        tree[index].m=tree[index].mr=tree[index].ml=tree[index].s=val; return ;
    }
    int m = (s+e)/2;
...
int main()
{
    ...
    while(scanf("%d",&n)==1)
    {
        ...
        for(int i=0; i<c;i++)
        {
        int j,k,f; scanf("%d %d %d",&f,&j,&k);
        if(f==1) printf("%lld\n",query(tree,index,s,e,j-1,k-1).m);
        else updateNode(tree,index,s,e,j-1,k);
        }
    }
    return 0;
}
